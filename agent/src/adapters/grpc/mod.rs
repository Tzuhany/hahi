...
use hahi_proto::chat::{
    ...
    ResumeRunRequest, SendMessageRequest, SendMessageResponse,
};
...
impl AgentExecutionService for AgentState {
    async fn send_message(
        ...
        self.runs
            .mark_running(&req.conversation_id, cancel.clone())
            .await;
        ...
        match output {
            Ok(output) => {
                self.runs.apply_output(&req.conversation_id, &output).await;
            ...
            Err(e) => {
                self.runs.mark_idle(&req.conversation_id).await;
    ...
    async fn resume_run(
        ...
        let checkpoint = self
            .load_checkpoint(&req.conversation_id)
            .await?
    ...
    async fn cancel_run(
        ...
        let cancelled = self.runs.cancel(&conversation_id).await;
    ...
    async fn get_run_status(
        ...
        if let Some(state) = self.runs.get(&conversation_id).await {
            return Ok(Response::new(GetRunStatusResponse {
                state: state.status.as_proto().to_string(),
                active_request_id: state.active_request_id,
            }));
        }

        let checkpoint = self.load_checkpoint(&conversation_id).await?;
        let (status, active_request_id) = checkpoint
            .and_then(|cp| {
                cp.pending_control
        ...
        Ok(Response::new(GetRunStatusResponse {
            state: status.as_proto().to_string(),
            active_request_id,
        }))
    }
}
