...
//
// Single entry point for all memory operations in run.rs.
...
//
// MemoryEngine does NOT handle reflection (that needs core/loop via run.rs)
...
use crate::memory::{inject, lifecycle, recall, reflect};
