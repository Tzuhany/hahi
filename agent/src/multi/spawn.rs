...
// They reuse the same agent loop (core/loop.rs) with different config.
//
...
use crate::core::r#loop::{LoopConfig, LoopEvent, LoopRuntime, TurnResult, run_loop};
use crate::multi::agent_def::find_agent_def;
use crate::multi::fork::build_fork_messages;
use crate::multi::isolation::build_sub_agent_config;
...
pub async fn spawn_sub_agent(
    ...
    let result = tokio::spawn(async move {
        let hooks = crate::core::HookRunner::empty();
    ...
    let status = match &result.stop_reason {
        crate::core::r#loop::TurnStopReason::Completed => "completed",
        crate::core::r#loop::TurnStopReason::Cancelled => "stopped",
        _ => "failed",
    };

    let duration_ms = start_time.elapsed().as_millis() as u64;
    let notification = crate::core::xml::task_notification_with_usage(
        &task_id,
        status,
        &output,
        result.usage.total(),
