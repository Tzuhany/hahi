...
use crate::core::context::{ContextManager, ContextPressure};
use crate::core::error_recovery::{
    self, ApiErrorKind, LoopControl, RecoveryAction, RecoveryState, continuation_prompt,
    effective_max_tokens,
};
use crate::core::event_bus::EventBus;
use crate::core::hooks::{HookDecision, HookEvent, HookRunner};
use crate::core::permission::{PermissionDecision, PermissionEvaluator};
use crate::core::pipeline::{CompressionPipeline, StageEvent};
use crate::core::plan_mode;
use crate::infra::metrics::Metrics;
use crate::llm::{LlmProvider, ProviderConfig};
use crate::tool::ToolRegistry;
use crate::tool::executor::{CompletedTool, PendingTool, ToolExecutor};
...
impl LoopConfigBuilder {
    ...
    pub fn build(self) -> Arc<LoopConfig> {
        let compression =
            crate::core::pipeline::CompressionPipeline::standard(Arc::clone(&self.provider), None);
        Arc::new(LoopConfig {
            compression,
...
impl LoopRuntime {
    ...
    pub fn take_pending_control(&self) -> Option<PendingControl> {
        self.pending_control.lock().ok().and_then(|mut slot| slot.take())
    }
}
    ...
    loop {
        ...
        if pressure >= ContextPressure::Compact {
            for event in config
                ...
                .await
            {
                runtime.bus.emit(match event {
                    StageEvent::Collapsed { folded_count } => LoopEvent::Collapsed { folded_count },
                    StageEvent::Compacted { pre_tokens } => LoopEvent::Compacted { pre_tokens },
                });
            }
        }
...
async fn dispatch_tool(
    ...
    match config.permission.check(&name, &input) {
        PermissionDecision::Deny => {
            ...
            bus.emit(LoopEvent::HookBlocked {
        ...
        PermissionDecision::Ask => {
            ...
            bus.emit(LoopEvent::SessionStateChanged {
                state: "requires_action".into(),
            });
            bus.emit(LoopEvent::ControlRequest {
                request_id: request_id.clone(),
                kind: "permission".into(),
                payload,
            });
...
async fn finalize_turn(
    ...
    let stop_reason = if config.run_mode == RunMode::Planning {
        let request_id = new_id();
        let plan = crate::core::xml::plan(&final_text);
        let payload = serde_json::json!({
            "plan": plan,
            "steps": [],
        });
