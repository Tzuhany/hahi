...
use crate::kernel::r#loop::LoopEvent;
        ...
        tokio::spawn(async move {
            while let Some(event) = child_rx.recv().await {
                let forwarded = match event {
                    ...
                    // These events are local to the sub-agent — not forwarded.
                    // Listed explicitly so adding a new LoopEvent variant requires
                    // a conscious decision here rather than silently dropping it.
                    LoopEvent::SessionStateChanged { .. }
                    | LoopEvent::ControlRequest { .. }
                    | LoopEvent::Stream(_)
                    | LoopEvent::PlanModeChanged { .. }
                    | LoopEvent::TurnEnd { .. } => None,
                };
