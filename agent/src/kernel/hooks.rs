...
impl<A: Hook, B: Hook> Hook for OrHook<A, B> {
    ...
    /// Allow if ANY handling hook allows. Non-handling hooks are neutral.
    ///
    /// Evaluation:
    ///   - If left handles and allows → Allow immediately (short-circuit)
    ///   - If right handles and allows → Allow
    ///   - If both block → return the left block (or right if left didn't handle)
    ///   - If neither handles → Allow (shouldn't happen; handles() guards the call)
    async fn evaluate(&self, event: &HookEvent) -> HookDecision {
        let left = if self.left.handles(event) {
            Some(self.left.evaluate(event).await)
        } else {
            None
        };

        if matches!(left, Some(HookDecision::Allow)) {
            return HookDecision::Allow;
        }

        let right = if self.right.handles(event) {
            Some(self.right.evaluate(event).await)
        } else {
            None
        };

        if matches!(right, Some(HookDecision::Allow)) {
            return HookDecision::Allow;
        }

        // Neither allowed — return the block from whichever handled.
        match (left, right) {
            (Some(block), _) => block,
