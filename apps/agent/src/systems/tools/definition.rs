...
pub trait Tool: Send + Sync {
    ...
    ///
    /// Read-only tools (web search, memory recall) default to `true`.
    /// Tools with side effects (memory write, agent spawn) return `false`
    /// and run sequentially after all concurrent tasks complete.
