...
/// Static configuration for one agent loop run.
///
/// Constructed once per turn via [`LoopConfigBuilder`] and wrapped in `Arc`
/// so sub-agents can share it without cloning large fields.
///
/// Separated from [`LoopRuntime`] (per-turn mutable state) so the static
/// config can be cheaply passed to async tasks without lifetime issues.
pub struct LoopConfig {
    // ── Core: what the loop calls ─────────────────────────────────────────────
    /// LLM provider (Anthropic, OpenAI, …). Sub-agents share the same instance.
    pub provider: Arc<dyn LlmProvider>,
    /// All tools available for this run (may be filtered by `run_mode`).
    pub tool_registry: Arc<ToolRegistry>,
    /// Fully assembled system prompt. Includes tool schemas, skills, memory index.
    pub system_prompt: String,

    // ── LLM call parameters ───────────────────────────────────────────────────
    /// Model ID (e.g., `"claude-opus-4-6"`).
    pub model: String,
    /// Maximum tokens in the LLM's response. Escalated on MaxTokens stops.
    pub max_tokens: u32,
    /// Sampling temperature. `None` uses the provider's default.
    pub temperature: Option<f32>,
    /// Provider-specific extensions (e.g., Anthropic's `betas` array).
    pub provider_extensions: HashMap<String, serde_json::Value>,

    // ── Context window management ─────────────────────────────────────────────
    /// Model's total context window in tokens (used for pressure detection).
    pub context_window_tokens: usize,
    /// Fraction of `context_window_tokens` at which compaction triggers (default 0.7).
    pub compact_threshold: f64,
    /// Compression pipeline (tool-result budget → context collapse → LLM summarize).
    /// Built once and reused across iterations.
    pub compression: CompressionPipeline,

    // ── Execution policy ──────────────────────────────────────────────────────
    /// Maximum tool-use iterations before the loop stops with `MaxIterations`.
    pub max_iterations: u32,
    /// Whether the loop has full tool access, read-only (planning), or reflection-only.
    pub run_mode: RunMode,
    /// Per-tool permission evaluator. Sub-agents inherit this from the parent.
    pub permission: Arc<PermissionEvaluator>,

    // ── Sub-agent / distributed context ──────────────────────────────────────
    /// Working directory for filesystem tools. Defaults to `/tmp` if not set.
    pub cwd: std::path::PathBuf,
    /// Position in the sub-agent spawn tree (depth, chain ID, max depth).
    pub chain: QueryChain,
    /// Shared metric counters. All sub-agents in a chain write to the same instance.
...
/// Returns the model to use, preferring `LLM_MODEL` env var over [`DEFAULT_MODEL`].
...
/// Atomic counters for one agent turn.
///
/// Shared between the main loop and sub-agents via `Arc`.
/// All operations use `Relaxed` ordering — precise counts matter, not ordering guarantees.
#[derive(Default)]
...
impl LoopStats {
    /// Record that a memory was written during this run.
    /// Used by `reflect.rs` to decide whether post-turn reflection is needed.
    pub fn record_memory_write(&self) {
        self.memories_written_this_run
            .fetch_add(1, Ordering::Relaxed);
    }

    /// Total memories written since this run started.
            ...
            StreamOutcome::Done {
                ...
                match &action {
                    ...
                    RecoveryAction::CompactAndRetry => {
                        ...
                        if events
                            .iter()
                            .any(|e| matches!(e, StageEvent::Compacted { .. }))
                        {
                            config.metrics.record_compact();
                        }
                    }
