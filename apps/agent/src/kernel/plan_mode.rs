...
use crate::kernel::r#loop::RunMode;
use crate::systems::tools::registry::ToolRegistry;

/// Read-only tools allowed in plan mode.
/// These can observe but not modify — safe for exploration.
///
/// Also used by `PermissionMode::Readonly` to determine which tools to auto-allow.
pub(crate) const PLAN_MODE_TOOLS: &[&str] = &[
    "WebSearch",
    "WebFetch",
...
pub fn format_plan_feedback(feedback: &str) -> String {
    crate::kernel::xml::plan_feedback(feedback)
}
