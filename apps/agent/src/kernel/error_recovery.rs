...
pub struct RecoveryState {
    ...
    /// When true, the low-output auto-stop is disabled.
    ///
    /// Use for long-running tasks where the user has explicitly accepted the
    /// token cost and consecutive low-output turns are expected (e.g., research
    /// that produces many short intermediate steps before a large final result).
...
pub fn evaluate(
    ...
    if !state.ignore_diminishing_returns {
...
mod tests {
    ...
    #[test]
    fn test_diminishing_returns_skipped_when_opted_out() {
        let mut state = RecoveryState {
